...
# Show a live window with raw YOLO detections (set False to disable)
...
# ==================== Low-latency smoothing + 3D constant-velocity KF ====================
class OneEuro3D:
...
class Kalman3D:
    ...
    def __init__(self, q_pos=1e-4, q_vel=1e-3, r_meas=2e-4):
        ...
        # store base Q to allow temporary scaling while moving
...
# ============================== Tracker (MINIMAL CHANGES) ===============================
class PegStereoMatcher:
    """
    Your original logic, with only these changes:
      - "Fast mode" while the moving token is active → lighter smoothing + higher Q.
      - Publish guard is DISABLED only for the moving peg (prevents freeze).
      - Supports passing precomputed YOLO results (to avoid double inference for the viz window).
    """
    def __init__(self):
        self.max_pegs = 6
        ...
        # derived thresholds you had
        self.gate_static = self.movement_threshold * 5.0    # ~15
        self.gate_moving = self.movement_threshold * 25.0   # ~50
        self.publish_guard_radius = 10.0

        # motion token hysteresis (keep your originals)
        self.speed_on  = 0.020
        self.speed_off = 0.010
        self.token_on_frames  = 1
        self.token_off_frames = 6

        # --------- NEW: Fast-motion relaxers (ONLY while moving token is active) ---------
        self.fast_min_cutoff = 5.0     # OneEuro: higher cutoff → less smoothing when moving
        self.fast_beta       = 0.2    # OneEuro: reacts more to motion
        self.slow_min_cutoff = 1.0     # your original
        self.slow_beta       = 0.02    # your original
        self.Q_moving_mult   = 25.0    # KF process noise multiplier while moving
        self.disable_guard_when_moving = True  # bypass two-frame publish guard for moving peg only
        # ---------------------------------------------------------------------------------
    ...
    def _init_tracks(self, points_3d):
        ...
        for pid, pos in enumerate(points_3d):
            ...
            eu = OneEuro3D(freq=120.0, min_cutoff=self.slow_min_cutoff, beta=self.slow_beta, d_cutoff=1.0)
            self.pegs[pid] = {
    ...
    def _pick_token(self):
        # speed-based hysteresis (unchanged)
        moving = []
        for pid, st in self.pegs.items():
        ...
        if self.moving_peg_id is not None:
            ...
            if st['static_frames'] >= self.token_off_frames and np.linalg.norm(st['kf'].vel()) < self.speed_off:
                # peg has settled; record stable pos and drop token
    ...
    def _associate(self, meas):
        """
        Your original Euclidean, unique assignment with static-first then moving token.
        """
        assigned = {}
        used_meas = set()
    ...
    def _update_tracks(self, assigned, meas, tnow, dt):
        for pid, st in self.pegs.items():
            # --------- NEW: adaptive filter ONLY for moving peg ----------
            if pid == self.moving_peg_id:
                st['kf'].Q = st['kf'].Q_base * self.Q_moving_mult
                st['euro'].min_cutoff = self.fast_min_cutoff
                st['euro'].beta = self.fast_beta
            else:
                st['kf'].Q = st['kf'].Q_base
                st['euro'].min_cutoff = self.slow_min_cutoff
                st['euro'].beta = self.slow_beta
            # -------------------------------------------------------------
            ...
            if pid in assigned:
                ...
                st['missing_frames'] += 1
                # If static and occluded, freeze at last published to avoid drift
                if pid != self.moving_peg_id and st['missing_frames'] <= self.max_missing_frames:
                    z = np.array(st['position'])
                    st['kf'].update(z.reshape(3))
                # if moving and occluded, rely on prediction

            # Smoothed state
            sm = st['euro'].filter(st['kf'].pos(), tnow)

            # --------- MODIFIED PUBLISH: disable guard ONLY while moving ----------
            if pid == self.moving_peg_id and self.disable_guard_when_moving:
    ...
    def update(self, left_frame, right_frame, res_l=None, res_r=None):
        # detections (allow caller-provided results to avoid double inference)
        ...
        self._update_tracks(assigned, points_3d, tnow, dt)
...
if __name__ == "__main__":
    try:
        ...
        while cap.isOpened():
            ...
            # Single YOLO pass for both viz + tracker
            res_l = model(left_frame,  verbose=False, conf=0.10)[0]
            res_r = model(right_frame, verbose=False, conf=0.10)[0]
            ...
            # Update peg tracking with SAME detections (no double inference)
        ...
        right_sock.close()
        if SHOW_DETECTIONS:
            cv2.destroyWindow("YOLO detections (L|R)")
